...
def process_video(
    ...
    video_name = os.path.splitext(os.path.basename(input_video_path))[0].replace("_splatting_results", "") + "_inpainting_results"
...
def main(
    ...
    if os.path.isfile(input_video_path):
        video_files = [input_video_path]
    else:
        video_files = [os.path.join(input_video_path, f) for f in os.listdir(input_video_path) if f.endswith('.mp4')]
